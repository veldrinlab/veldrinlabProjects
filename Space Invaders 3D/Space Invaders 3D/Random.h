...
/**
 * Class reprsenst Random class which is used to calculate random int and float values.
 */
class Random
{
private:
...
public:
	...
	/**
	 * Default constrcutor, set seed to random 42 value :).
	 */
	Random()
	{
		seed = 42;
	...
	/**
	 * Constrcutor with initialize parameter.
	 ...
	 */
	Vector3D getPointOnCircle(const float r = 1.0f)
	{
		float angle = getAngle();

		return Vector3D(r * cos(angle), r * sin(angle), 0.0f);
	}
	 ...
	 */
	Vector3D getVector3D(const float minX,const  float maxX,const  float minY,const  float maxY, const float minZ,const float maxZ)
	{
		return Vector3D(getFloat(minX, maxX), getFloat(minY, maxY), getFloat(minZ, maxZ));
	}
};
#endif
