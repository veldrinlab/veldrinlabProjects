...
{
	actualTime = 0.0f;
	previousTime = static_cast<float>(SDL_GetTicks()/1000.0f);
	elapsedTime = 0.0f;
...
/**
 * Default destructor, nothind to do.
 */
Timer::~Timer()
{
...
{
	actualTime = static_cast<float>(SDL_GetTicks()/1000.0f);
...
/**
 * Method used to check if second has elapsed.   
